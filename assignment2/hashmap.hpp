...
public:
   ...
   */
  ~HashMap() noexcept;  // noexcept for RAII
   ...
   */
  inline size_t size() const;  // always const for primitive types
   ...
   *      auto [iter1, insert1] = map.insert({3, "Avery"}); // inserts {3, "Avery"}, iter1 points to
   * that element, insert1 = true auto [iter2, insert2] = map.insert({3, "Anna"});  // no-op, iter2
   * points to {3, "Avery"}, insert2 = false
   *
   ...
   * other than iterators to the erased K/M element.
   */
  iterator erase(const_iterator pos);  // union iterator?
   ...
   * need to know is that they are very limited, and have three functions: init.begin(), init.end(),
   * and init.size(). There are no other ways to access the elements in an initializer_list. As a
   * result, you probably want to leverage the range constructor you wrote in the previous function!
   *
  ...
  auto operator=(const HashMap& map) -> HashMap&;
...
private:
   ...
   */
  std::vector<node*> _buckets_array;  // fixed size and changed by rehash()
  void copy_buckets_array(const decltype(_buckets_array)& source);  // added helper function
