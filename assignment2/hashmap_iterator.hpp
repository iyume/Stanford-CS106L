...
public:
  /*
   * This alias is very important. When dealing with const_iterator, the value_type is always const,
   * and that prevents the client from modifying the elements via a const_iterator. The
   * meta-function std::conditional_t changes the value_type (at compile-time) to a const one if
   * IsConst is true.
   */
  using value_type =
      std::conditional_t<IsConst, const typename Map::value_type, typename Map::value_type>;
  ...
  /*
   * Friend declarations so the HashMap class this iterator is for can access the attributes of its
   * iterators. Also, to make conversions between iterator and const_iterator easy, we declare the
   * corresponding iterator and const_iterators as friends.
   */
  friend Map;
   ...
   *      iter++;         // postfix
   *
   * Note: the prefix operator first increments, and the returns a reference to itself (which is
   * incremented). The postfix operator returns a copy of the original iterator, while the iterator
  ...
  friend bool operator==(const HashMapIterator<Map_, IsConst_>& lhs,
                         const HashMapIterator<Map_, IsConst_>& rhs);
  ...
  /*
   * Special member functions: we explicitly state that we want the default compiler-generated
   * functions. Here we are following the rule of zero. You should think about why that is correct.
   */
...
private:
  ...
  /*
   * Instance variable: pointer to the node that stores the element this iterator is currently
   * pointing to.
   */
  node* _node;
...
HashMapIterator<Map, IsConst>::HashMapIterator(bucket_array_type* buckets_array,
                                               node* node,
                                               size_t bucket)
...
/*
 * operator-> is a bit of an odd-ball. When you write p->m, it is interpreted as
 * (p.operator->())->m. This means operator-> should return a pointer to the object that has the
 * field m.
 *
 ...
 * object that has the 'first' and 'second' field, i.e. a pointer to the std::pair/the value of that
 * node.
 */
template <typename Map, bool IsConst>
...
template <typename Map, bool IsConst>
bool operator==(const HashMapIterator<Map, IsConst>& lhs,
                const HashMapIterator<Map, IsConst>& rhs) {
